  ...
  private inicializarListenerClientes(): void {
    ...
    getDoc(userRef).then(userSnap => {
      ...
      if (roles.includes('GIMNASIO')) {
        ...
        getDoc(doc(this.firestore, 'entrenadores', userId)).then(entrenadorSnap => {
          if (entrenadorSnap.exists()) {
            const entrenadorData = entrenadorSnap.data();
            const gimnasioId = entrenadorData?.['gimnasioId'];
            if (gimnasioId) {
              const q = query(clientesRef, where('gimnasioId', '==', gimnasioId));
              this.configurarSnapshotListener(q);
            } else {
              this._loading.set(false);
              this._clientes.set([]);
            }
        ...
        return; // Retornamos aquí porque la consulta se configura en el callback
      ...
      this.configurarSnapshotListener(clientesQuery);
    }).catch(error => {
